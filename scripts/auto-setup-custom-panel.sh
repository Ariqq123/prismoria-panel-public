...
# Fresh machine installer for a custom Pterodactyl panel backup.
# Supports backup archives in .zip or .tar.gz format.
#
# What it does:
# 1) Installs required packages (optional).
# 2) Extracts backup archive to /.
# 3) Configures domain + nginx + optional Let's Encrypt TLS.
# 4) Ensures DB/user exists and updates .env DB values.
# 5) Applies ownership/permissions and verifies critical write access.
# 6) Restarts panel-related services.
...
usage() {
    cat <<'USAGE'
Usage:
  sudo bash scripts/auto-setup-custom-panel.sh --archive /path/to/custom-panel.zip --domain panel.example.com [options]

Required:
  --archive PATH                 Backup archive path (.zip, .tar.gz, .tgz, .tar)
...
Database options:
  ...
  --db-name NAME                 DB name (default: read from restored .env)
  --db-user USER                 DB user (default: read from restored .env)
  --db-pass PASS                 DB password (default: read from restored .env or generated)
...
Runtime options:
  ...
  --skip-migrations              Skip php artisan migrate
  --build-assets                 Run yarn install + yarn build:production
  --help                         Show this help text

Examples:
  sudo bash scripts/auto-setup-custom-panel.sh \
    --archive /root/download/pterodactyl-custom-panel-20260304.zip \
    --domain panel.example.com \
    --letsencrypt-email admin@example.com

  sudo bash scripts/auto-setup-custom-panel.sh \
    --archive /root/download/pterodactyl-custom-panel-20260304.zip \
    --checksum /root/download/pterodactyl-custom-panel-20260304.zip.sha256 \
    --domain panel.example.com \
    --db-name panel --db-user pterodactyl --db-pass 'StrongDBPass'
USAGE
}
...
sanitize_redis_config() {
    ...
    # Some hosts end up with invalid locale directives in redis.conf.
    # Redis fails hard on these unknown directives.
    ...
    # Redis 7.0 does not support set-max-listpack-* directives from newer configs.
...
verify_php_runtime_modules() {
    if ! php -m | rg -qi '^redis$'; then
        fail "PHP redis extension is missing. Install php${PHP_VERSION}-redis (or php-redis), then retry."
    fi
}
...
extract_archive() {
    ...
    case "${ARCHIVE_PATH}" in
        ...
        *.tar)
            ...
            ;;
        *)
            fail "Unsupported archive format: ${ARCHIVE_PATH}. Use .zip, .tar.gz, .tgz, or .tar."
            ;;
    esac
}
...
ensure_database_credentials() {
    ...
    [[ -n "${DB_NAME}" ]] || fail "Unable to determine DB name. Pass --db-name."
    [[ -n "${DB_USER}" ]] || fail "Unable to determine DB user. Pass --db-user."
...
write_nginx_config() {
    ...
    [[ -S "${php_socket}" ]] || fail "PHP-FPM socket not found: ${php_socket}"
...
apply_permissions() {
    log "Applying ownership and writable permissions ..."
...
verify_permissions() {
    log "Verifying permissions and write access ..."
    [[ -d "${PANEL_DIR}" ]] || fail "Panel path missing: ${PANEL_DIR}"
    [[ -d "${PANEL_DIR}/storage" ]] || fail "storage directory missing."
    [[ -d "${PANEL_DIR}/bootstrap/cache" ]] || fail "bootstrap/cache directory missing."
    [[ -f "${PANEL_DIR}/artisan" ]] || fail "artisan missing."

    local owner
    owner="$(stat -c '%U:%G' "${PANEL_DIR}")"
    [[ "${owner}" == "www-data:www-data" ]] || fail "Panel owner mismatch (${owner}). Expected www-data:www-data."

    runuser -u www-data -- test -w "${PANEL_DIR}/storage" || fail "www-data cannot write ${PANEL_DIR}/storage"
    runuser -u www-data -- test -w "${PANEL_DIR}/bootstrap/cache" || fail "www-data cannot write ${PANEL_DIR}/bootstrap/cache"
    runuser -u www-data -- test -r "${PANEL_DIR}/.env" || fail "www-data cannot read ${PANEL_DIR}/.env"
}
...
patch_websocket_proxy_service() {
    local svc="/etc/systemd/system/pterodactyl-external-ws-proxy.service"
    [[ -f "${svc}" ]] || return 0

    if command_exists node; then
        local node_path
        node_path="$(command -v node)"
        sed -i -E "s#^ExecStart=.*external-websocket-proxy\\.js#ExecStart=${node_path} ${PANEL_DIR}/scripts/external-websocket-proxy.js#g" "${svc}"
    fi
    sed -i -E "s#^WorkingDirectory=.*#WorkingDirectory=${PANEL_DIR}#g" "${svc}" || true
}
...
case "${ARCHIVE_PATH}" in
    *.zip)
        command_exists unzip || fail "unzip is required for .zip archives."
        ;;
    *.tar.gz|*.tgz|*.tar)
        command_exists tar || fail "tar is required for tar archives."
        ;;
esac
...
PRE_BACKUP_DIR="/root/panel-autosetup-preflight-${TS}"
mkdir -p "${PRE_BACKUP_DIR}"

log "Creating pre-install safety backup at ${PRE_BACKUP_DIR} ..."
...
cp -a /etc/systemd/system/pteroq.service "${PRE_BACKUP_DIR}/" 2>/dev/null || true
...
[[ -f "${PANEL_DIR}/artisan" ]] || fail "Invalid restore: artisan missing."
...
# Re-assert ownership after root-level install/build steps.
apply_permissions
verify_permissions
...
if [[ -n "${LETSENCRYPT_EMAIL}" ]]; then
    echo "TLS: Let's Encrypt enabled (email: ${LETSENCRYPT_EMAIL})"
else
    echo "TLS: not requested (HTTP only)"
fi
echo "Pre-install safety backup: ${PRE_BACKUP_DIR}"
if [[ "${DB_PASS_GENERATED}" -eq 1 ]]; then
    echo "Generated DB password: ${DB_PASS}"
fi
echo
echo "Recommended checks:"
echo "  1) systemctl status nginx php${PHP_VERSION}-fpm redis pteroq pterodactyl-external-ws-proxy --no-pager"
echo "  2) php ${PANEL_DIR}/artisan p:environment:mail"
echo "  3) Visit the panel and test local + external server routes."

...
class ServerConfigurationStructureService
{
    ...
    public function handle(Server $server, array $override = [], bool $legacy = false): array
    {
        ...
        $allowLegacy = (bool) config('pterodactyl.legacy.allow_legacy_server_configuration', true);
        if ($legacy && !$allowLegacy) {
            Log::warning('Legacy server configuration format requested while disabled; returning current format.', [
                'server_id' => $server->id,
            ]);

            return $this->returnCurrentFormat($clone);
        }

        return $legacy ? $this->returnLegacyFormat($clone) : $this->returnCurrentFormat($clone);
    }
    ...
    protected function returnCurrentFormat(Server $server): array
    {
        ...
        if ((bool) config('pterodactyl.legacy.emit_container_oom_disabled', true)) {
            $container['oom_disabled'] = $server->oom_disabled;
        }

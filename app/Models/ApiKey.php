...
class ApiKey extends Model
{
    ...
    public static function findToken(string $token): ?self
    {
        ...
        if (is_null($model)) {
            Log::debug('API key lookup failed: identifier not found.', ['identifier' => $identifier]);

            return null;
        }

        try {
            $decrypted = decrypt($model->token);
        ...
        if (hash_equals($decrypted, substr($token, strlen($identifier)))) {
            return $model;
        }

        Log::debug('API key lookup failed: token mismatch.', [
    ...
    public static function allowsLegacyApplicationAuthorization(): bool
    {
        return (bool) config('pterodactyl.legacy.allow_application_api_keys', true);
    }

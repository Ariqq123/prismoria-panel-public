...
class BackupManager
{
    ...
    protected function resolve(string $name): FilesystemAdapter
    {
        ...
        if (isset($this->customCreators[$adapter])) {
            return $this->callCustomCreator($adapter, $config);
        }
     ...
     */
    protected function callCustomCreator(string $adapter, array $config): FilesystemAdapter
    {
        $instance = $this->customCreators[$adapter]($this->app, $config);
        Assert::isInstanceOf($instance, FilesystemAdapter::class);

...
{
    protected const FILE_PATH_REGEX = '/^\/api\/client\/servers\/((?:[a-z0-9-]{36})|(?:external:[0-9]+:[^\/]+))\/files(\/?$|\/(.)*$)/i';
    ...
    protected function configureRateLimiting(): void
    {
        ...
        RateLimiter::for('external-panel', function (Request $request) {
            $userKey = optional($request->user())->uuid ?: $request->ip();
            $routeValue = (string) ($request->route('externalServer') ?? $request->route('connection') ?? 'global');
            $connectionId = explode(':', $routeValue, 2)[0] ?: 'global';
            $path = (string) $request->getPathInfo();

            // Large folder uploads can generate many short-lived API requests. Keep a
            // stricter default limit for general external actions, and apply a larger
            // bucket only to upload-related endpoints.
            if (preg_match('/\/files\/(?:upload(?:\/proxy)?|create-folder)\/?$/i', $path) === 1) {
                return Limit::perMinutes(
                    config('http.rate_limit.external_panel_upload_period', 1),
                    config('http.rate_limit.external_panel_upload', 900)
                )->by("$userKey:$connectionId");
            }

            return Limit::perMinutes(
                config('http.rate_limit.external_panel_period', 1),
                config('http.rate_limit.external_panel', 120)
            )->by("$userKey:$connectionId");
        });
    }
}

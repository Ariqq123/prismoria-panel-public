...
const server: ServerDataStore = {
    ...
    inConflictState: computed((state) => {
        ...
        const isExternal = state.data.source === 'external';
        const hasConflictStatus =
            status === 'installing' ||
            status === 'install_failed' ||
            status === 'reinstall_failed' ||
            status === 'suspended' ||
            (!isExternal && status === 'restoring_backup');

        return hasConflictStatus || state.data.isTransferring || state.data.isNodeUnderMaintenance;
    }),

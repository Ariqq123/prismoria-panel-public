    ...
    <>
      ...
      {blueprintRoutes.server.length > 0 &&
        blueprintRoutes.server
          ...
          .filter((route) => {
            if (
              serverIdentifier.startsWith('external:') &&
              (route.identifier === 'subdomainmanager' || route.identifier === 'votifiertester' || route.identifier === 'mclogs')
            ) {
              return true;
            }

            return extensionEggs[route.identifier].includes('-1')
              ? true

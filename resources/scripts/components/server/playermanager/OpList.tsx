...
export default ({ players, search, onUpdate, selectedKey, onSelect }: Props) => {
  ...
  return (
    <Section>
      <SectionHeader>
        <SectionTitle>OP List</SectionTitle>
        <SectionBadge>{filteredPlayers.length} Entries</SectionBadge>
      </SectionHeader>
      <SectionBody>
        {filteredPlayers.length < 1 ? (
          <EmptyState>
            {players.length < 1 ? 'There are no operators on the list.' : 'No operators match your current filter.'}
          </EmptyState>
        ) : (
          filteredPlayers.map((item, key) => {
            const inspectable = buildInspectablePlayer(item, 'op');
            ...
            return (
              <ListItem key={`${inspectable.key}-${key}`} $selected={isSelected}>
                ...
                <Actions>
                  ...
                  <AskModal
                    title={'DEOP Player'}
                    message={`Are you sure that you want to deop <b>${item.name}</b>?`}
                    buttonText={'DEOP'}
                    command={`deop ${item.name}`}

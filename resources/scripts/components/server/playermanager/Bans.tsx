...
export default ({ players, search, onUpdate, selectedKey, onSelect }: Props) => {
  ...
  return (
    <Section>
      <SectionHeader>
        <SectionTitle>Banned Players</SectionTitle>
      ...
      <SectionBody>
        {filteredPlayers.length < 1 ? (
          <EmptyState>
            {players.length < 1
              ? 'There are no banned players on the list.'
              : 'No banned players match your current filter.'}
          </EmptyState>
        ) : (
          filteredPlayers.map((item, key) => {
            const inspectable = buildInspectablePlayer(item, 'ban');
            ...
            return (
              <ListItem key={`${inspectable.key}-${key}`} $selected={isSelected}>
                ...
                <Actions>
                  ...
                  <AskModal
                    title={'Pardon Player'}
                    message={`Are you sure that you want to pardon <b>${item.name}</b>?`}
                    buttonText={'Pardon'}
                    command={`pardon ${item.name}`}
